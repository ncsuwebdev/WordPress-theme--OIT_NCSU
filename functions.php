...
$config = array(
	...
	'brand_strip_color'  => 'red_on_white',// options: black, red, red_on_white, black_on_white
	'header_bg' => 'white', // Header and Footer Background => options: black, red, white
	'header_color' => 'red' // options: black, red, white
	
);

// config_options takes a string as a parameter and calls get_config_options() 
function config_options($show) {
	echo get_config_options($show);
}

// get_config_options take a string as a parameter and returns the appropriate member of the $config array
function get_config_options($show) {
	
    global $config;
	switch( $show ) {
		case 'brand_strip_color' :
			$output = $config['brand_strip_color'];
			break;
		case 'header_bg' :
			$output = $config['header_bg'];
			break;
		case 'header_color' :
			$output = $config['header_color'];
			break;
	}
	return $output;
}	

// adds custom menu support for two menus
/* Multiple menus for theme */
add_action( 'init', 'addMyMenus' );
...
if ( function_exists( 'add_image_size' ) ) { 
	add_image_size( 'wide-featured-thumbnail', 794, 9999 ); //for 2-col layout; 790 pixels wide (and unlimited height)
	add_image_size( 'featured-thumbnail', 794, 9999 ); //for homepage featured slider
}
...
function sandbox_body_class( $print = true ) {
	global $wp_query, $current_user, $config;

	// It's surely a WordPress blog, right?
	$c = array('wordpress');
	
	if (empty($config['header_bg']) || $config['header_bg'] == NULL) {
		$config['header_bg'] = 'white';
		$config['header_color'] = 'red';
		$c[] = $config['header_bg'].'bg';
	} else if (($config['header_bg'] === 'red') || ($config['header_bg'] === 'black')) {
		$c[] = 'bg '. $config['header_bg'].'bg';
	}
...
// Generate the appropriate layout for pages
function generate_layout_id () {

	if (is_home()) return 'two_col_vert'; else return 'three_col_vert';	
	
}

// Modification to default excerpt settings for the homepage feature
// Change default excerpt length
function custom_excerpt_length( $length ) {
	return 80;
}
add_filter( 'excerpt_length', 'custom_excerpt_length', 999 );
// Change default read-more text and link to the post
